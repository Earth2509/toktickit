...
| BR-09 | Actions Taken are append-only: no Action is deleted and its Ticket, original performer and creation audit fields never change. An Action status moves only from `OPEN` to `COMPLETED` or `CANCELLED`; a terminal Action is not reopened, so a later correction is recorded as a new Action. The original performer or an Administrator may correct permitted mutable work fields, assignment or status using the record version; every correction/status change creates an audit event. |
| BR-10 | Requesters never change formal Ticket status. A requester resolution indication remains advisory. |
| BR-11 | `RESOLVED` requires an active owner, at least one completed Action Taken, no open Action Taken, no completed Action whose follow-up remains required, and a resolution summary. `CLOSED` requires `RESOLVED`. This gate is re-evaluated after reopen, so historical Actions alone cannot resolve a reopened Ticket. |
