...
| [#15](https://github.com/Earth2509/toktickit/issues/15) / [PR #23](https://github.com/Earth2509/toktickit/pull/23) | Create Ticket screen and submission states | @Nuggetkub ran the branch and confirmed the claimed test count, validation bounds, busy protection, API-failure preservation, and backend-assigned Ticket values. He withheld approval because a mixed valid/invalid attachment selection discarded valid files and did not name the rejected file or reason. He also noted misleading post-success attachment wording, the staging-target `Closes #15` wording, silent upper-bound truncation, and an undocumented idempotency-key lifecycle. | The form now partitions attachment selections: it retains permitted files, lists each rejected filename with a type or size reason, and tests the mixed-selection result. The success copy now says that files were not uploaded; upper bounds remain visible and validation-backed with live counts; and the idempotency-key lifecycle is documented. The pull-request closing wording will be corrected before re-review. | Follow-up changes implemented; re-review pending. |
