...
| P-07 | "เริ่มทำส่วนต่อไปได้" ("You can start the next part") | Implement Issue #38: authenticated authorization for the retained Requester workflows, Login/mandatory password-change UI, removal of client-supplied identity, regression tests and review-ready evidence. |
...
Implemented the authentication foundation and its Issue #38 integration: preserving User migration, session/password/CSRF APIs, local-only fixtures, same-origin proxying, Login and mandatory password change, role-aware landing/navigation, session-expiry handling, and server-derived Requester ownership across ticket and attachment operations. The development Requester selector and directory were removed. Lab 2 identity-injection tests were replaced by authenticated API/UI/E2E equivalents while pure validation/query tests remained. Real unit, API, client, migration and browser runs exposed and corrected a stale Requester-selector E2E flow, an ambiguous password-field locator, and a Windows-only TypeScript runner startup problem; E2E preparation now uses compiled JavaScript and the production start path matches the compiler output.
