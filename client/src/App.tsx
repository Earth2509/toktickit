...
function AuthenticatedWorkspace({ session, onSessionChanged, onLoggedOut }: { session: AuthSession; onSessionChanged: (session: AuthSession) => void; onLoggedOut: () => void }) {
  const [view, setView] = useState<"create" | "tickets" | "detail" | "queue" | "staff-detail" | "password">(session.user.role === "IT_STAFF" || session.user.role === "ADMINISTRATOR" ? "queue" : "tickets");
